...
def procces_quad(op):
    ...
    while semantic.PilaOper and prec.get(semantic.PilaOper[-1], 0) >= prec.get(op, 0):
        clean_quad()
...
def p_clean_program(p):
    ...
    semantic.QuadList.append(["ENDPROGRAM", None, None, None])
    semantic.QuadListMemory.append(["ENDPROGRAM", None, None, None])
...
def p_print_element_expr(p):
    ...
    #Quads print Int or Float or Variable
...
def p_factor(p):
    '''factor : grouping
              ...
              | cte
              | ID id_func_option'''
    if len(p) == 3 and p[2] == "fcall":
        return
    elif len(p) == 3 and p[2] is None:
        #If is ID append to pila operandos and appends its type
        ...
        #If is cte append to pila operandos and appends its type
        semantic.PilaO.append(p[1])
...
def p_grouping(p):
    'grouping : LPARENTESIS push_paren expression RPARENTESIS close_paren'
    #Asociation taking parenteses into account
    pass
...
def p_cte(p):
    ...
    if p.slice[1].type == 'CTE_INT':
        # THIS IS SO THAT FACTOR CAN TAKE THE VALUE AND MAKE THE QUADS
        val = int(p[1])
    else:
        val = float(p[1])
...
def p_start_func_vars(p):
    ...
    #print(f"Step 10, VarTable created for func '{semantic.current_function}'")
    #ESTO LLO TENGO QUE HACER????
    pass
...
def p_f_call(p):
    ...
    semantic.call_stack.pop()
    pass

def p_make_era(p):
    'make_era :'
    func_name = p[-2]
    #semantic.call_function = func_name lo que tenai antes
    semantic.call_stack.append(func_name)
...
def p_make_go_sub(p):
    'make_go_sub :'
    func = semantic.call_stack[-1]
    gosub_value = semantic.func_dir.directory[func]['startLine']
    semantic.QuadList.append(["GOSUB", None, None, gosub_value])
    semantic.QuadListMemory.append(["GOSUB", None, None, gosub_value])
    ret_type = semantic.func_dir.directory[func]['type']
    if ret_type != 'void':
        temp = semantic.new_temp()
        # memoria del temporal
        if ret_type == 'int':
            temp_addr = memory.allocate("TempInt", temp)
        elif ret_type == 'float':
            temp_addr = memory.allocate("TempFloat", temp)
        else:
            temp_addr = memory.allocate("TempBool", temp)

        semantic.QuadList.append(["=", func, None, temp])
        semantic.QuadListMemory.append(["=", func, None, temp_addr])
        semantic.PilaO.append(temp)
        semantic.PilaT.append(ret_type)

    #semantic.call_function = None

    if semantic.PilaOper and semantic.PilaOper[-1] == '(':
        semantic.PilaOper.pop()
...
def p_declare_param(p):  #check parameters type
    'declare_param :'
    current_call = semantic.call_stack[-1]   # ESTA ES LA FUNCION QUE SE ESTA LLAMANDO
    
    if semantic.parameter_check >= len(semantic.func_dir.directory[current_call]['params']):
        return
    
    if not semantic.PilaO:
        raise TypeError("Semantic error: missing argument value for parameter")
    
    parameter = semantic.PilaO.pop()
    parameter_type = semantic.PilaT.pop()
    
    i = semantic.parameter_check
    
    expected_type = semantic.func_dir.directory[current_call]['params'][i]

    if expected_type != parameter_type:
        raise TypeError(f"cannot call funct with those parameters")

    param_name = list(semantic.func_dir.directory[current_call]['vars'].table.keys())[i]
    param_addr = semantic.func_dir.get_address(current_call, param_name)
    parameter_addr = semantic.func_dir.get_address(semantic.current_function, parameter)

    semantic.QuadList.append(["PARAMETER", parameter, None, param_name])
    semantic.QuadListMemory.append(["PARAMETER", parameter_addr, None, param_addr])
