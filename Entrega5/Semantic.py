...
class VarTable:
    ...
    def add(self, var_name, var_type, address):
        ...
        self.table[var_name] = { 'type': var_type, 'address': address }
...
class FunctionDirectory:
    ...
    def add_var(self, func_name, var_name, var_type):
        ...
        if func_name == prog_name and var_type == 'int':
            #Global int
            addr = memory.allocate("GlobalInt", var_name)
        elif func_name == prog_name and var_type == 'float':
            #Global int
    ...
    def get_var_type(self, func_name, var_name):
        try:
            ...
            pass
        global_vars_scope = list(self.directory.keys())[0] # nombre del programa, checar como se hace
    ...
    def get_address(self, func_name, var_name):
        ...
        """if isinstance(var_name, str):
            if var_name in memory.ConstantString:
                return memory.ConstantString[var_name]
            else:
                return memory.allocate('ConstantString', var_name)"""
...
def print_quads():
    ...
    print("\nCUADRUPLOS FINALES memorias sin numeros")
    for i, q in enumerate(QuadListMemory):
        print(f"{q}")
